    ...
    class WPH_Widget extends WP_Widget
    {
        ...
        */
        ...
        function create_field_textarea( $key, $out = "" )
        {
            ...
            if ( isset( $key['class'] ) )
                $out .= 'class="tinymce-enabled ' . esc_attr( $key['class'] ) . '" ';
         ...
         * @since    1.5
         */
        function create_field_wysiwyg( $key, $out = "" )
        {
            $out .= $this->create_field_label( $key['name'], $key['_id'] ) . '<br/>';
        ...
        */
        ...
        function create_field_media_picker( $key )
        {
            $value = isset( $key['value'] ) ? $key['value'] : $key['std'];
            $out = '
            <script>
                jQuery(document).ready(function() {
                    jQuery(\'#'. $key["_id"] .'_button\').click(function() {
                        formfield = jQuery(\'#'. $key["_id"] .'\').attr(\'name\');
                    ...
                    window.send_to_editor = function(html) {
                        ...
                        jQuery(\'#'. $key["_id"] .'\').val(imgurl);
                        tb_remove();
                    }
                });
            </script>
            <label for="'. $key["_id"] .'">'.$key['name'].':</label><br>
            <input id="'. $key["_id"] .'" type="text" size="36"';
            if ( isset( $key['class'] ) )
                $out .= 'class="' . esc_attr( $key['class'] ) . '" ';
            $out .=' name="' . esc_attr( $key['_name'] ) . '" value="'.$value.'" />
            <input id="'. $key["_id"] .'_button" type="button" value="Upload Image" />
            ';
            if ( isset( $key['desc'] ) )
                $out .= '<br/><small class="description">'.esc_html( $key['desc'] ).'</small>';
